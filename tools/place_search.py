...
class PlaceSearchTool:
    ...
    def _setup_tools(self) -> List:
        ...
        @tool
        def search_fooditem(place:str) -> str:
            """Search local food of a place """  
            try:
                fooditem_result = self.google_places_search.google_search_localfooditem(place)
                if fooditem_result:
                    return f"Following are the local food item presnt in {place} as suggested by google: {fooditem_result}"
            except Exception as e:
                tavily_result = self.tavily_search.tavily_search_fooditem(place)
                return f"Google cannot find the details due to {e}. \nFollowing are the local food item presnt in {place}: {tavily_result}"  ## Fallback search using tavily in case google places fail
        
       
       
        return [search_attractions, search_restaurants, search_activities, search_transportation,search_fooditem]
